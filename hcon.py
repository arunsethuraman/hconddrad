...
work_dir='/home/tuf29140/work/HconddRAD/'
data_dir=work_dir
...
model_file = data_dir+'hcon.model'

logfile = '/home/tuf29140/work/HconddRAD/testppp.log'
...
convert.run(['--vcf',filtered_vcf_name,'--out-format','binary-ped','--out-prefix',vcf_dir+'hcon','--overwrite'])

#Convert chromosome names
rc = subprocess.call("./convertchromosomes.sh")

#awk '{$1=0;print $0}' hcon.bim > hcon.bim.tmp
#mv hcon.bim.tmp hcon.bim

for k in range (1, 10):
	admixture.run(['--binary-ped-prefix',vcf_dir+'hcon','--pop','%d'%(k)])
	graph_plotter.bar_plot('hcon.%d.Q'%(k))
print ("Plots created")
